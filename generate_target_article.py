...
# 找到目標查詢
target_query = "微量吸管的使用方法與技巧"
target_outline = None

for outline in outlines_data["outlines"]:
    if outline["query"] == target_query:
        target_outline = outline
        break

if not target_outline:
    print(f"❌ 找不到查詢：{target_query}")
    print(f"可用查詢列表：")
    for outline in outlines_data["outlines"]:
        print(f"  - {outline['query']}")
    sys.exit(1)
...
# 顯示配置的字數要求
print(f"\n📋 字數要求（來自 settings.yaml）：")
for block_name, block_cfg in config.article_blocks.items():
    min_words = block_cfg.get('word_count_min', 0)
    max_words = block_cfg.get('word_count_max', 0)
    if block_name == 'faq':
        q_min = block_cfg.get('questions_min', 0)
        q_max = block_cfg.get('questions_max', 0)
        ans_max = block_cfg.get('answer_max_words', 0)
        print(f"   {block_name}: {q_min}-{q_max} 問題，每個回答 ≤{ans_max} 字")
    else:
        print(f"   {block_name}: {min_words}-{max_words} 字")
